...
node_labs_tbl <-
  ...
  right_join(
    tibble(
      node = Ntip(sanger_tree_rooted) + seq_along(sanger_tree_rooted$node.label)
    ), by = "node"
  ) %>%
  mutate(
    fossil_label = replace_na(fossil_label, ""),
    calibration_type = replace_na(calibration_type, "none")) %>%
  arrange(node)
...
fossil_map_plot <- 
  ggtree(
    sanger_tree_rooted,
    branch.length = "none", size = 0.1) %<+% node_labs_tbl +
  # Add node points
  geom_nodepoint(
    aes(color = calibration_type), size = 1) +
  scale_color_manual(
    values = c(
