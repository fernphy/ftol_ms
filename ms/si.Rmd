...
accepted_species %>%
	...
	anti_join(select(ppgi_taxonomy_mod, genus, notes)) %>%
...
ppgi_taxonomy_mod %>%
	...
	transmute(
		...
		genus,
		# FIXME: need to add n species from PPGI and note
		n_species = NA,
		note = NA
	)
...
Differences in genera included as accepted in Pteridocat taxonomic database (https://github.com/pteridophy/pteridocat) vs. @PteridophytePhylogenyGroupI2016 (`r result_file(s_table("genera_diff"), "csv") %>% fs::path_file()`).
"diff" is type of difference (Pteridocat genus not in PPGI or PPGI genus not in Pteridocat).
"n_species" is the number of accepted species in each genus in the system that includes the genus.
