...
```{r bs-plot}
# Parse bootstrap values from IQTREE
phy <- sanger_con_tree_rooted

bs_values <-
tibble(
  label_orig = phy$node.label
) %>%
  mutate(label_new = case_when(
    label_orig == "Root" ~ NaN,
    TRUE ~ parse_number(label_orig) * 0.01
  )) %>%
  pull(label_new)
...
**`r s_figure("bs")`**.
Consensus fern phylogeny with node labels bootstrap support (%) (`r result_file(s_figure("bs"), "pdf") %>% fs::path_file()`).
