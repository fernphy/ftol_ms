	...
	data %>%
		mutate(
			...
			plot_group = case_when(
				plot_group %in% c("Ophioglossales", "Psilotales") ~ 
...
Ages of fern families differed from previous studies, with some families estimated as having older crown ages while some had younger (`r figure("divtimes")`).
...
plastome_gleich_genera_n <-
	bb_sampling %>%
	filter(plot_group == "Gleicheniales") %>%
	pull(genus) %>%
	n_distinct()
```
...
Although we recovered slightly older ages for ferns and other large clades relative to the most recent globally sampled fern phylogeny [@Testo2016a], our estimated ages for families do not show a clear pattern, with some older and some younger than other studies (`r figure("divtimes")`).
...
Therefore, it is perhaps not surprising that we obtained different estimates for the age of many families, whereas previous studies tended to estimate similar ages by re-using the same (or very similar), more limited set of calibration points (SI Fig 1).
...
clade_cols <-
	...
	extract(seq_len(nrow(plot_group_exemp_taxa))) %>%
	set_names(plot_group_exemp_taxa$plot_group)

# Format coverage at order-ish level for plotting
b <-
	coverage_data %>%
	filter(rank == "plot_group") %>%
	left_join(plot_group_exemp_taxa, by = c(name = "plot_group")) %>%
	arrange(phy_order) %>%
	rename(clade = name) %>%
	mutate(
...
clade_tibble <-
	...
	pivot_longer(names_to = "plot_group", values_to = "taxa", cols = everything()) %>%
	arrange(plot_group) %>%
	group_by(plot_group) %>%
...
suborder_labels <-
	...
	mutate(
		clade_label = case_when(
			plot_group == "Aspleniineae" ~ "Aspleniineae\n(eupolypods II)",
			plot_group == "Polypodiineae" ~ "Polypodiineae\n(eupolypods I)",
			TRUE ~ plot_group
		)
	)
...
**`r figure("divtimes")`**. Crown age of fern families (mya) estimated by selected studies.
