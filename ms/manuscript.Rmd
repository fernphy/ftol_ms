...
The "plants" division of GenBank release 249 (cutoff date 2022-04-15) was downloaded from the NCBI FTP server (https://ftp.ncbi.nlm.nih.gov/genbank/) and a local database including only ferns and outgroups (selected seed plants, lycophytes, bryophytes, and algae) created using the restez R package [@Bennett2018a].
...
After implementing a manual inclusion list for Thelypteridaceae and consulting with a taxonomic expert on this family (Susan Fawcett), the number of non-monophyletic genera dropped to `r top_nonmono_subfam %>% filter(family == "Thelypteridaceae") %>% pull(n)`.
