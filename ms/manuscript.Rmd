...
We also applied two constraints outside of ferns, including the most recent common ancestor (MRCA) of euphyllophytes (`r fossils$Euphyllophytes$age` Ma) and MRCA of tracheophytes (`r fossils$Tracheophytes$age` Ma), and fixed the root age (land plants) at 475 Ma as in @Testo2016a.
...
Code used to generate FTOL and compile this manuscript is available at https://github.com/pteridophylo/ftol_flow and https://github.com/pteridophylo/ftol_ms, respectively.
...
The final selection of Sanger loci (one set of concatenated accessions per species or one accession per species if conditions for concatenation were not met) included `r n_accs_raw_final` accessions (`r n_accs_final` sequences when counting distinct loci within each accession separately; `r s_table("accs")`) representing `r n_species_sanger` species (not including species in the plastome dataset).
...
Selection of the longest set of concatenated sequences per species resulted in `r n_accs_plast_final` accessions (species; `r s_table("accs")`).
...
We envision that FTOL will be integrated with the next iteration of PPG, PPG II, to provide the most recent hypotheses on the monophyly of various fern taxa, which will in turn enable a more natural classification system.
