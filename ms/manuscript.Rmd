...
tibble(species = plastome_tree$tip.label) %>%
	mutate(
		...
		phy_order = seq_len(nrow(.))) %>%
	left_join(select(ppgi_taxonomy, family, genus), by = "genus") %>%
	mutate(family = replace_na(family, "og")) %>%
	group_by(family) %>%
	slice(1) %>%
	ungroup() %>%
	arrange(phy_order)
	
backbone_tree <- ape::keep.tip(plastome_tree, family_exemp$species)
backbone_tree$tip.label <- family_exemp$family

backbone_tree$node.label <- str_replace(backbone_tree$node.label, "100", "")
...
plot(backbone_tree, font = 1)
nodelabels(backbone_tree$node.label, frame = "none", col = "grey", cex = 0.8)
```
