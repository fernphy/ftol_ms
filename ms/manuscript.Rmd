...
# Mean bootstrap for consensus tree
tibble(
  bs = sanger_con_tree_rooted$node.label)
...
The best-scoring model according to BIC selected for the backbone phylogeny was `r extract_iqtree_mod(plastome_iqtree_log)`.
The tree search completed after `r extract_iqtree_iter(plastome_iqtree_log)` iterations, at which point the bootstrap correlation coefficient of split occurrence frequencies was `r extract_iqtree_corr(plastome_iqtree_log)`.
...
Bootstrap support was generally across the tree (mean BS) and particularly high at deeper nodes.
Relationships within some more recently diverged clades were less well-supported, including *Cyathea*, *Amauropelta*, and parts of *Dryopteris* and *Elaphoglossum* (`r s_figure("bs")`).
