...
sanger_tree <- ape::read.tree(here::here("working/sanger_alignment.phy.tre"))
```
...
## Automated removal of rogue sequences
...
We generated a backbone phylogeny using maximum likelihood (ML) analysis of the plastome dataset in IQ-TREE [@Nguyen2015] with `r number(1000)` ultrafast rapid bootstrap replicates [@Minh2013].
We applied the GTR+I+G model to a concatenated dataset with no partitioning.

### Initial Sanger phylogeny

Next, we used the backbone phylogeny as a constraint tree to conduct initial phylogenetic analysis of the concatenated Sanger dataset in IQ-TREE in with the "-fast" option and same model settings as the backbone phylogeny. We prioritized computation speed for this step, as we needed to repeat it multiple times as described in the next section.

### Manual inspection of initial Sanger phylogeny

We manually inspected the initial Sanger phylogeny to identify any remaining rogues or names that needed updating in the taxonomic database.
We checked for monophyly at taxonomic levels at or above genus using the R package "MonoPhy" [@Schwery2016].
We updated our taxonomic database if the molecular data clearly indicated the current usage of a synonym was incorrect [`r eg`, a taxonomic intruder into an otherwise expected monophyletic genus with a synonym available for that genus\; @Schwery2016].
However, this was not possible in all cases, particularly in groups that are in need of taxonomic revision (`r eg`, cheilanthoid ferns, grammitid ferns, microsoroid ferns).
While we consider our phylogeny as an important guide for future taxonomic revisions, we did not make any taxonomic changes that would require valid publication according to the International Code of Nomenclature for Algae, Fungi, and Plants.
In a very small number of cases we assigned a different higher level taxonomy for recently described names if no existing combinations were available that were compatible with our taxonomic system.
These are indicated in our supplemental data files as "comb. ined.".

Based on the results of the manual inspection, we updated the list of rogue accessions to exclude, updated the taxonomic database, then re-ran all analyses to this step.
...
Therefore, we repeated this process until the expected monophyly of all higher-level (`r eg` genus rank and higher) taxa that have previously been shown to be monophyletic was confirmed.

### Final Sanger phylogeny

After manual inspection, we generated the final Sanger phylogeny by using the backbone phylogeny as a constraint tree in ML analysis of the Sanger dataset under otherwise identical settings as those used for the backbone phylogeny.

We did not conduct a more exhaustive analysis testing various partitions of the data or other phylogenetic inference methods as our goal is to produce a single species-level phylogeny that is a reasonable hypothesis of fern evolution, not to interrogate the outcomes of multiple, more or less equally applicable methods.

## Molecular dating

We dated the tree using treePL [@Smith2012].
We applied a set of FIXME calibration points as minimum ages.
Our calibration points are all based on fossils (not secondary calibration points) following a careful literature review.
We used the species identification of each fossil to assign lineage only after consulting the original publication; previous studies [`r eg`, @Testo2016a] have used genus to assign lineage, but this can be problematic as genus concepts may vary between the original publication and current taxonomy [`r eg`, *Kuylisporites*\; @Loiseau2020].
We tested rate smoothing parameters ranging from 0.000001 to 1000 (each varies by one order of magnitude); a value of FIXME was selected with the smallest chi-squared value for final analysis [@Smith2012].
...
Within Aspleniineae, we recover two large clades.
...
Unlike most other published fern phylogenies, the current version of FTOL described in this manuscript (`r ftol_ver`) is not meant to be the last.
...
fern_tree_sampling <-
	tribble(
		~source, ~year, ~n_species,
		"Hasebe1995", 1995, 107,
		"Schuettpelz2007", 2007, 400,
		"Lehtonen2011", 2011, 2957,
		"Testo2016a", 2016, 3973,
		"this_study", 2021, n_sanger_ferns
	) %>%
...
```{r genbank-print, fig.width = 3.3, fig.height = 5, eval = params$doc_type == "pdf"}
...
**`r figure("ftol")`**. Fern Tree of Life (FTOL) `r ftol_ver`. Inset plot shows coverage by major clade (order or suborder). Numbers next to each bar show sampled species out of total number of species. Taxonomy follows [@PteridophytePhylogenyGroupI2016].
