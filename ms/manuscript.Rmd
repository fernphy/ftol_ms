...
We did not conduct a more exhaustive analysis testing various partitions of the data or other phylogenetic methods as our goal is to produce a single species-level phylogeny that is a reasonable hypothesis of fern evolution, not to interrogate the outcomes of multiple, more or less equally applicable phylogenetic methods.

## Manual inspection

After generating the initial ML tree, we manually inspected it to identify any putative remaining rogues at finer levels that were missed by our automated detection at the family level.
Where applicable, we updated our taxonomic database if the molecular data clearly indicated the current usage of a synonym was incorrect.
However, this is not possible in all cases, particularly in groups that are in need of taxonomic revision (`r eg`, cheilanthoid ferns, grammitid ferns, microsoroid ferns).
While we consider our phylogeny as an important guide for future taxonomic revisions, we did not make any taxonomic changes that would require valid publication according to the Code of Botanical Taxonomy.
...
We compiled a list of putative rogue GenBank accessions, re-ran all analyses to this point after excluding rogues, then continued with the manually inspected phylogeny.
...
ggsave(
	...
	genbank_plot, width = 3.3, height = 5, units = "in")
```
