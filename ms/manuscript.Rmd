...
Ferns (`r ca` `r number(12000)` species) are the second most diverse clade of vascular plants after angiosperms (`r ca` `r number(300000)` species) and are a useful study system for understanding processes of biogeography [@Tryon1986; @Kato1993], community ecology [`r eg`, @Hennequin2014; @Lehtonen2015], and speciation [`r eg`, @Kao2020].
...
Indeed, growth of plastid fern accessions in GenBank show no sign of slowing since the most recent global fern phylogeny [@Testo2016a\; `r figure("genbank")`].
...
A local database was then created from these data including only ferns and outgroup taxa (select seed plants, lycophytes, bryophytes, and algae) using the restez R package [@Bennett2018a].
GenBank accession numbers corresponding to sequences potentially matching each target locus were obtained by querying the GenBank with terms including the locus name (`r eg`, "rbcL"), "Polypodiopsida[ORGN]", and a date cutoff matching the current GenBank release.
...
We also implemented a manual approach for Thelypteridaceae, which had the highest number of non-monophyletic genera (16) in a previous version of FTOL (v1.0.0) and likely has high rates of misidentification on GenBank.
...
Dating was conducted separately on the ML tree and the consensus tree using penalized likelihood as implemented in treePL [@Smith2012].
...
We tested rate smoothing parameters in treePL ranging from `r smooth_start` to `r smooth_end` (each varies by one order of magnitude); these values are based on consideration that our tree spans a wide phylogenetic range with high rate heterogeneity, and results of initial analyses in which higher smoothing values sometimes produced spurious date estimates.
...
In particular, superSMART [@Antonelli2016] and pyPHLAWD [@Smith2019a] are recently developed pipelines that can generate maximally sampled phylogenies for any higher taxon of choice.
...
While we were unable to integrate these data into the current version of FTOL, they will be automatically added when the next version of GenBank data is released, and we expected the next version of FTOL will reflect this new knowledge.
