...
The current FTOL (v0.0.1) includes `r comma(n_sanger_ferns)` species, an increase of ca. 20% relative to the most recent global fern phylogeny while accounting for synonyms.
...
The set of plastome genes is based on the list of 83 protein-coding genes of @Wei2017a, which was filtered to only genes that show no evidence of duplication (77 genes).
...
For these, we selected one specimen per species with the longest combined sequence length across all plastome genes.
