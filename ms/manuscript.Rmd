...
accepted_species <-
	...
	left_join(select(ppgi_taxonomy, genus, family, order, class), by = "genus") %>%
...
backbone_tree_full <- 
	plastome_tree %>%
	ape::root.phylo("Anthoceros_angustus") %>%
	ape::ladderize() %>%
	ape::drop.tip(og_taxa)

family_exemp <-
tibble(
	species = backbone_tree_full$tip.label
) %>%
	mutate(genus = str_split(species, "_") %>% map_chr(1)) %>%
	left_join(select(ppgi_taxonomy, family, genus), by = "genus") %>%
	group_by(family) %>%
	slice(1) %>%
	ungroup() %>%
	pull(species)

backbone_tree_exemp <- ape::keep.tip(backbone_tree_full, family_exemp)

bb_trans_table <-
	tibble(species = backbone_tree_exemp$tip.label) %>%
	mutate(genus = str_split(species, "_") %>% map_chr(1)) %>%
	left_join(select(ppgi_taxonomy, family, genus), by = "genus") %>%
	left_join(accepted_sampling_by_rank, by = c(family = "name")) %>%
	assert(not_na, species, family, n) %>%
	select(tip.label = species, clade.label = family, N = n) %>%
	## set crown node depth to 1/2 the maximum depth
	mutate(
		depth = sapply(backbone_tree_exemp$tip.label, function(x,y) 
	0.5*y$edge.length[which(backbone_tree_exemp$edge[,2] ==
	which(y$tip.label == x))], y = backbone_tree_exemp)
	)

## convert
tt <- phylo.toBackbone(backbone_tree_exemp, bb_trans_table)
## plot
plot(tt)
```
